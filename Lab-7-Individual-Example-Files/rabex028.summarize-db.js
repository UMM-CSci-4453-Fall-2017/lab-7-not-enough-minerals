...
sql = "SHOW DATABASES";
connection.query(sql, function(err, rows,fields) {//connecion.connect is run automatically for a query
  if(err){
...
function processDBFs(dbfs){// Asynchronous row handler
  ...
  }// don't put a connection.end() here it will kill all queued queries.
}
